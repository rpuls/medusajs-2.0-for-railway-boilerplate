...
export const config = {
  ...
  //
  // sitemap.xml and robots.txt are excluded because they are NOT
  // region-prefixed: they are metadata routes at the app root, and without this
  // the middleware would 307 a crawler from /sitemap.xml to /gb/sitemap.xml,
  // which does not exist. Both are generated by src/app/sitemap.ts and
  // src/app/robots.ts.
  matcher: [
    "/((?!api|_next/static|favicon.ico|sitemap.xml|robots.txt|.*\\.png|.*\\.jpg|.*\\.gif|.*\\.svg).*)",
  ],
}
