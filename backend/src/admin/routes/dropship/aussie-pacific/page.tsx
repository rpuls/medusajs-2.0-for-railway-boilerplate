...
// Mirrors backend/src/admin/routes/dropship/page.tsx but targets the
// Aussie Pacific endpoints. AP exposes no status-polling API, so the Sent
// tab shows the snapshot at the time of submission and an explanatory
// banner — operators reconcile shipments via AP's email/portal.
      ...
      {open && (
        <div className="flex flex-col gap-y-3 border-t border-ui-border-base px-4 py-4">
          <div className="flex flex-col gap-y-1">
            <Label htmlFor="ap-drop-shipping-method" className="text-xs">
              Shipping method
            </Label>
            <Input
              id="ap-drop-shipping-method"
          ...
          <div className="flex flex-col gap-y-1">
            <Label htmlFor="ap-drop-notes" className="text-xs">
              Delivery notes
            </Label>
            <Textarea
              id="ap-drop-notes"
              ...
              onChange={(e) =>
                onChange({ ...settings, deliveryNotes: e.target.value })
              }
              rows={2}
              maxLength={1000}
            />
    ...
    for (let i = 0; i < selectedOrders.length; i++) {
      ...
      try {
        const res = await fetch(
          `/admin/orders/${order.order_id}/send-to-aussie-pacific`,
          {
            method: "POST",
            credentials: "include",
            headers: {
            ...
            body: JSON.stringify({
              ...
              deliveryNotes: settings.deliveryNotes || undefined,
            }),
          }
        )
    ...
    return (
      <Container className="flex flex-col items-center gap-y-3 py-12">
        <Text className="text-ui-fg-muted">
          No pending Aussie Pacific orders — all caught up!
        </Text>
      </Container>
    )
  }
    ...
    <div className="flex flex-col gap-y-4">
      ...
      <div className="flex items-center justify-between">
        <Text size="small" className="text-ui-fg-subtle">
          {selected.size} of {orders.length} order
          {orders.length !== 1 ? "s" : ""} selected
        </Text>
        <Button
          variant="primary"
        ...
        >
          {sending
            ? "Sending…"
            : `Send selected (${selected.size}) to Aussie Pacific`}
        </Button>
      </div>
    </div>
  )
...
const SentTab = ({ orders }: { orders: SentOrder[] }) => {
  if (orders.length === 0) {
    return (
      <Container className="flex flex-col items-center gap-y-3 py-12">
        <Text className="text-ui-fg-muted">
          No Aussie Pacific orders sent yet.
        </Text>
      </Container>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-y-3">
      <Container className="rounded border border-ui-border-base bg-ui-bg-subtle p-3">
        <Text size="xsmall" className="text-ui-fg-subtle">
          Status updates not available via API — Aussie Pacific does not expose
          an order-status endpoint. Reconcile shipment progress via AP email
          confirmations or the distributor portal.
        </Text>
      </Container>
      ...
      <Container className="p-0 overflow-hidden">
        <div className="flex items-center gap-x-3 px-4 py-2 border-b border-ui-border-base bg-ui-bg-subtle text-ui-fg-subtle text-xs font-medium uppercase tracking-wide">
          ...
          <span className="w-32">AP ID</span>
        ...
        {orders.map((order) => {
          const color = statusBadgeColor(order.supplier_status)
          const shipments = order.supplier_shipments ?? []
          ...
          return (
            ...
            >
              ...
              <div className="w-24 shrink-0 text-sm text-ui-fg-subtle">
                {fmtDate(order.supplier_sent_at)}
              </div>
              ...
              <div className="flex-1 text-sm text-ui-fg-subtle">
                ...
                {order.supplier_last_error && (
                  <div className="text-ui-fg-error mt-1 text-xs">
                    Error: {order.supplier_last_error}
                  </div>
                )}
              </div>
            </div>
...
const AussiePacificDropshipPage = () => {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch("/admin/dropship/aussie-pacific", {
        credentials: "include",
      ...
      <Container className="flex items-start justify-between">
        <div>
          <Heading level="h1" className="flex items-center">
            Aussie Pacific Orders
            <HelpTooltip
              text={{
                title: "Aussie Pacific Orders",
                body: "Manage dropship fulfilment through Aussie Pacific. Orders containing Aussie Pacific SKUs appear here automatically. Pending orders have not been sent to AP yet; Sent orders have been forwarded via the AP API.",
                bullets: [
                  "Select one or more pending orders and click 'Send to Aussie Pacific' to submit them for fulfilment.",
                  "Use Shared send settings to set a default shipping method and delivery notes applied to every send in this session.",
                  "AP does not expose an order-status endpoint — reconcile shipment status via their email confirmations or distributor portal.",
                  "Only orders with Aussie Pacific line items appear here; orders with mixed suppliers need manual split-fulfilment.",
                ],
              }}
            />
          </Heading>
          <Text size="small" className="text-ui-fg-subtle mt-1">
            All orders with Aussie Pacific items from the past 90 days. Select
...
export const config = defineRouteConfig({
  label: "Aussie Pacific Orders",
  icon: ArrowPath,
})

export default AussiePacificDropshipPage
